...
{
    //All six are a package deal under firsts resource id
    bool f;
    Texture cubeMapTexture = CreateTexture(textureType, *textureResources[0], &f, true);
    if(f)
    {
    ...
    for(int i = 1; i < 6; i++)
    {
        ...
        Texture tempTexture = CreateTexture(textureType, *textureResources[i], &f, false);
    }
...
{
    bool f;
    Texture tempTexture = CreateTexture(textureType, textureResource, &f);
    if(!f){
...
Texture TextureManager::CreateTexture(unsigned int textureType, TextureResource& textureResource, bool* found, bool genTexture)
{
    *found = false;
    int n = textures.size();
    for(int i = 0; i < n; i++)
    ...
    Texture tempTexture;
    tempTexture.resourceID = textureResource.resourceID;
