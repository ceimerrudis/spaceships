...
//OBLY FOR DEBUG
unsigned int LoadShaderDEBUG(string vertexFileName, string FragmentFileName)
{
    unsigned int shaders;
    GL(shaders = glCreateProgram());
    string a = GetShader(vertexFileName);
    const char*  vertexShader = a.c_str();
    GL(unsigned int vs = glCreateShader(GL_VERTEX_SHADER));
    GL(glShaderSource(vs, 1, &vertexShader, NULL));
    ...
    GL(glCompileShader(fs));
    CheckShaderCompileStatus(fs, FragmentFileName.c_str());
    GL(glAttachShader(shaders, vs));
    GL(glAttachShader(shaders, fs));
    GL(glLinkProgram(shaders));
    CheckProgramLinkStatus(shaders);
    GL(glValidateProgram(shaders));

    GL(glDeleteShader(vs));
    GL(glDeleteShader(fs));
    GL(glUseProgram(shaders));
