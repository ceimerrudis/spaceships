...
Mesh::Mesh(unsigned int nameSize, unsigned int vertexSize, unsigned int indexSize, unsigned int normalsSize)
:   name(nameSize),
    vertices(vertexSize),
...
{   
    ...
    std::shared_ptr<Mesh> mesh = std::make_shared<Mesh>(modelName.size(), vertices.size()*3, indices.size(), normals.size()*3);
