...
bool AreCoplanar(cat::Vector<float, 3> p1, cat::Vector<float, 3> p2, cat::Vector<float, 3> p3, cat::Vector<float, 3> p4);
bool CanFaceSeePoint(cat::Vector<float, 3>* vertexes, Face f, cat::Vector<float, 3> p);
bool CanFaceSeePoint(cat::Vector<float, 3> p1, cat::Vector<float, 3> p2, cat::Vector<float, 3> p3, cat::Vector<float, 3> pointToCheck);

bool CanFaceSeePoint(cat::Vector<float, 3> p1, cat::Vector<float, 3> p2, cat::Vector<float, 3> p3, cat::Vector<float, 3> pointToCheck)
{
    cat::Vector<float, 3> cross = (p3-p1) * (p2-p1);
    cat::Vector<float, 3> toPoint = pointToCheck - p1;
    return toPoint % cross > 0.000001;
}


/*std::shared_ptr<Mesh> GenAsteroid(unsigned int id)
{
    ...
    int A = 0, B = 1, C = 2;
    Vector<float, 3> normal = (verticies[B] - verticies[A]) * (verticies[C] - verticies[A]);
        ...
        for (int j = 0; j < fcount; j++)// Go through each face and find conflicts
        {
            if(CanFaceSeePoint(verticies, faces[j], verticies[i]))
            {
                ...
                if (it == conflictGraph.end()) {
                    conflictGraph.push_back({i, {j}});solar
                } else {
    ...
    for (int i = 0; i < conflictGraph.size(); i)//size decreases
    {
        ...
        if(n != 0)
        {
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                Vector<float, 3> oldnormal = (verticies[oldFace.p2] - verticies[oldFace.p1]) * (verticies[oldFace.p3] - verticies[oldFace.p1]);
                Face f1 = {a, b, c};
...
{
    Vector<float, 3> cross = (vertexes[f.p3]-vertexes[f.p1]) * (vertexes[f.p2]-vertexes[f.p1]);
