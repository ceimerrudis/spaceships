...
DebugDraw::DebugDraw(std::vector<float> vert, const Matrix<float, 4> projMat, Vector<float, 3> color)
{
    this->line = true;    
...
DebugDraw::DebugDraw(std::vector<float> vert, std::vector<unsigned int> incdic, const Matrix<float, 4> projMat, Vector<float, 3> color)
{
    this->line = false;    
...
void DebugDraw::Render(const Matrix<float, 4> transformMatrix, const Matrix<float, 4> viewMat)
{
    this->transformMatrix = transformMatrix;
    GL(glUseProgram(shaderKey));
    
    glUniformMatrix4fv(viewMatUniformLoc, 1, GL_TRUE, viewMat.data);
    glUniformMatrix4fv(worldTransfUniformLoc, 1, GL_TRUE, transformMatrix.data);
    GL(glBindVertexArray(vertexArrayObjectKey));
