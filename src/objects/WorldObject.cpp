...
void WorldObject::orthonormalize() {
    ...
    vector3 proj_u1_v2 = right * (up % right);
    vector3 ortho_v2 = up - proj_u1_v2;
    up = ortho_v2.Normalized();

    vector3 proj_u1_v3 = right* (forward % right);
    vector3 proj_u2_v3 = up * (forward % up);
    vector3 ortho_v3 = forward - proj_u1_v3 - proj_u2_v3;
    forward = ortho_v3.Normalized();
}

matrix4 WorldObject::GetRotMatrix()
{
